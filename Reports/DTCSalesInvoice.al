report 50120 "DTC Sales Invoice"
{
    UsageCategory = Administration;
    ApplicationArea = All;
    RDLCLayout = './Layaout/DTCSalesInvoice.rdlc';
    Caption = 'DTC Sales Invoice Header';
    ...
    dataset
    {
        ...
        dataitem(Header; "Sales Invoice Header")
        {
            ...
            column(MonedaPagoCliente; TipoMoneda) { }
            ...
            column(PruebaTipoDocumento; PruebaTipoDocumento) { }
            dataitem(Lineas; "Sales Invoice Line")
            {
                ...
                var
                    ...
                    KCPTaxDefinition: record "KCP Tax Definition";
                begin
                    ...
                    if DetTaxLedEntry.FindFirst() then begin
                        ...
                        repeat
                            ...
                            if TaxJuridiction."DTC DIAN Code" = '01' then begin
                    ...
                    Header.CalcFields(Amount, "Invoice Discount Amount");
                    DescuentoPorLinea += lineas."Line Discount Amount";
                    DescuentoGeneralFactura := Header."Invoice Discount Amount" + DescuentoPorLinea;
                    Total := (Header.Amount - DescuentoGeneralFactura) + ((VAT5 + VAT19) * -1);
                    NetoConRetenciones := Total - ReteIva - RETEFUENTE - RETEICA;
                end;
            }
            trigger OnAfterGetRecord()
            var                                            // REFERENCIAS CODIGO DE BARRA 1D BARCODE Y 2D QR CODE
                ...
                StringNosSeriesDecr, SubStringFac, SubStringDeb, SubStringDebPredefinido, SubStringAcentDebit : Text[150];
                Resultado1, Resultado2, Resultado3, Resultado4 : Integer;
            begin
                ...
                Series.Reset();
                Series.SetRange(code, header."No. Series");
                if Series.FindFirst() then begin
                    repeat
                        StringNosSeriesDecr := UpperCase(Series.Description);
                        SubStringFac := UpperCase('Factura');
                        SubStringDeb := UpperCase('Debito');
                        SubStringAcentDebit := UpperCase('débito');
                        SubStringDebPredefinido := UpperCase('NDVP');
                        Resultado1 := StrPos(StringNosSeriesDecr, SubStringFac);
                        Resultado2 := StrPos(StringNosSeriesDecr, SubStringDeb);
                        Resultado3 := StrPos(StringNosSeriesDecr, SubStringDebPredefinido);
                        Resultado4 := StrPos(StringNosSeriesDecr, SubStringAcentDebit);
                        if Resultado1 > 0 then begin
                            PruebaTipoDocumento := 'Factura electrónica de venta No';
                            if (Resultado2 > 0) or (Resultado3 > 0) or (Resultado4 > 0) then
                                PruebaTipoDocumento := 'Nota Debito Electronica de Venta No'
                ...
                If Header."Currency Code" = '' then
                    ...
                    TipoMoneda := Header."Currency Code";
            end;
        }
    }
    ...
    var
        ...
        Cude_Cufe: text[250];
        Des1: text[100];
        PruebaTipoDocumento: Text[150];
        DescripcionSeries1, DescripcionSeries2, DescripcionSeries3 : text[250];
        FirmaDigitalDocumento: Text[150];
    // VARIABLES TIPO TEXT FINAL -------------------------------------------------------------------------------
}